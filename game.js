...
let selectTimerDuration = 30000;
let selectTimerPhase = "";
...
function showScreen(name) {
  document.body.classList.add("is-screen-switching");
  Object.values(screens).forEach(screen => screen.classList.remove("is-active"));
  screens[name].classList.add("is-active");
  document.body.classList.toggle("is-authenticated", Boolean(currentUser));
  window.setTimeout(() => document.body.classList.remove("is-screen-switching"), 180);
}
...
function updateSelectTimer() {
  ...
  const progress = clamp(remainingMs / selectTimerDuration, 0, 1);
  ...
  if (remainingMs <= 0) {
    stopSelectTimer();
    if (isRankedBanPhase(matchPrepState())) submitCharacterBan(pendingBanKind || "none");
    else submitCharacterReady();
  }
}
...
function openPvpSetup() {
  ...
  ui.modeMessage.classList.remove("is-error");
  if (ui.pvpQueueMessage) {
    ui.pvpQueueMessage.textContent = "";
    ui.pvpQueueMessage.classList.remove("is-error");
  }
  showScreen("pvpQueue");
}
...
function contactDamagePair(a, b) {
  ...
  if (a.mageTransfer) {
    const transfer = a.mageTransfer;
    if (b !== transfer.owner) damage(b, transfer.damage, transfer.owner);
    if (!b.owner) b.stunTime = Math.max(b.stunTime || 0, 120);
    b.mageTransfer = { ...transfer, time: 180 };
    a.mageTransfer = null;
  }
  if (b.mageTransfer) {
    const transfer = b.mageTransfer;
    if (a !== transfer.owner) damage(a, transfer.damage, transfer.owner);
    if (!a.owner) a.stunTime = Math.max(a.stunTime || 0, 120);
    ...
    if (rift.voidFieldTime > 0) {
      ...
      if (rift.voidDamageTick <= 0) {
        ...
        targets.forEach(target => {
          ...
          damageCombatTarget(target, 7, rift.owner);
        });
      }
    }
  });
...
function clearMageElements(target) {
  ...
  target.mageLastElement = "";
}

function mageReactionFor(existingElement, nextElement) {
  const key = `${existingElement}+${nextElement}`;
  if (key === "electro+fire") return { name: "독성 폭발", color: "#a3e635" };
  if (key === "electro+wet") return { name: "전이 감전", color: "#38bdf8" };
  if (key === "wet+electro") return { name: "감전 지대", color: "#22d3ee" };
  if (key === "wet+fire") return { name: "수증기", color: "#7dd3fc" };
  if (key === "fire+electro") return { name: "과부하", color: "#fb923c" };
  if (key === "fire+wet") return { name: "증발", color: "#f97316" };
  return null;
}

function triggerMageReaction(owner, target, existingElement, nextElement, reaction) {
...
function applyMageElement(owner, target, element) {
  ...
  const existingElement = target.mageLastElement || Object.entries(elements).find(([key, ticks]) => key !== element && ticks > 0)?.[0] || "";
  if (existingElement && existingElement !== element) {
    const reaction = mageReactionFor(existingElement, element);
    clearMageElements(target);
    if (reaction) triggerMageReaction(owner, target, existingElement, element, reaction);
    return;
  }
  target.mageElements = {
    ...elements,
...
function updateMageAilments(target, dt) {
  ...
  if (target.mageToxicGas) {
    const gas = target.mageToxicGas;
    gas.time -= dt;
    gas.tick -= dt;
    while (gas.time > 0 && gas.tick <= 0 && !game.over) {
      damage(target, gas.damage, gas.owner);
      gas.tick += 60;
    }
...
function useRouletteNormal(owner) {
  ...
  if (kind === "charger") {
    ...
    throwBall(owner, { force: true, rouletteShot: true });
    const target = nearestEnemyTarget(owner);
    ...
    setRouletteForm(owner, "believer", 300);
    addVisualEffect({
      type: "ceremony-light",
      fighter: owner,
      color: owner.accent,
...
function triggerNormalSkill(fighter) {
  ...
  if (fighter.kind === "brawler") {
    if (fighter.hp <= fighter.maxHp * 0.5) {
      const missingFromHalf = Math.max(0, fighter.maxHp * 0.5 - fighter.hp);
      const healAmount = 10 + Math.min(10, missingFromHalf / (fighter.maxHp * 0.5) * 10);
      heal(fighter, healAmount);
    } else {
      fighter.brawlerSpeedStacks += 1;
      addSkillPulse(fighter, fighter.accent);
    }
    fighter.skillTimer = 840;
    return;
...
function moveFighter(fighter, dt) {
  ...
  if (fighter.kind === "archmage") {
    if (fighter.mageVaporHealTime > 0) {
    ...
    if (fighter.mageConductiveField) {
      fighter.mageConductiveTick -= dt;
      while (fighter.mageConductiveTick <= 0 && !game.over) {
        damage(opponentOf(fighter), 3, fighter);
  ...
  if (fighter.kind === "gambler") {
    ...
    if (fighter.rouletteFormTime > 0) {
      ...
      if (fighter.rouletteFormTime <= 0) {
        if (fighter.rouletteFormKind === "stealth") {
          fighter.stealthTime = 0;
  ...
  if (fighter.canThrow && !fighter.cosmicBlasterActive) {
...
function updateArtOrbs(dt) {
  ...
  game.artOrbs.forEach(orb => {
    ...
    if (!last || Math.hypot(last.x - orb.x, last.y - orb.y) > 18) {
      ...
      if (orb.trail.length > ART_TRAIL_MAX_POINTS) {
        orb.trail = orb.trail.filter((_, index) => index % 2 === 0);
      }
    }
  });
}
...
function useDrawing(owner) {
  ...
  targets.forEach(target => {
    ...
    for (let index = 2; index < orb.trail.length; index += 2) {
      const previous = orb.trail[index - 2];
...
function useInnerPeace(owner) {
  const orb = game.artOrbs?.find(item => item.owner === owner);
  if (!orb) return;
  const targets = [owner, opponentOf(owner), ...game.summons].filter(target => target?.hp > 0);
  targets.forEach(target => {
    const inTrail = orb.trail.some((point, index) => {
      if (index < 1) return false;
      const previous = orb.trail[index - 1];
      const width = Math.max(previous.radius || 18, point.radius || 18);
      return pointSegmentDistance(target.x, target.y, previous.x, previous.y, point.x, point.y) < target.radius + width;
    });
    if (inTrail) {
      const before = target.hp;
      target.hp = clamp(target.hp + 30, 0, target.maxHp);
      target.healingDone = (target.healingDone || 0) + Math.max(0, target.hp - before);
    }
  });
  updateHud();
  addVisualEffect({
...
function updateCosmicDusts(dt) {
  if (!game.cosmicDusts) game.cosmicDusts = [];
  game.fighters.forEach(fighter => {
  ...
  game.cosmicDusts = game.cosmicDusts.filter(dust => {
    if (dust.owner.hp <= 0) return false;
    dust.life -= dt;
    const angle = Math.atan2(dust.owner.y - dust.y, dust.owner.x - dust.x);
    const speed = 1.75;
    dust.x += Math.cos(angle) * speed * dt;
    dust.y += Math.sin(angle) * speed * dt;
    if (Math.hypot(dust.owner.x - dust.x, dust.owner.y - dust.y) < dust.owner.radius + dust.radius + 4) {
      dust.owner.cosmicDust += 1;
      return false;
    }
    return dust.life > 0;
  });
...
function drawCosmicDusts() {
  if (!game.cosmicDusts?.length) return;
  game.cosmicDusts.forEach(dust => {
    ctx.save();
    ctx.translate(dust.x, dust.y);
    ctx.rotate(game.tick * 0.08);
    ctx.globalCompositeOperation = "lighter";
...
function cosmicBlasterLine(owner) {
  const direction = fighterDirection(owner);
  return {
    x1: owner.x + direction.x * (owner.radius + 8),
    y1: owner.y + direction.y * (owner.radius + 8),
    x2: owner.x + direction.x * 900,
    y2: owner.y + direction.y * 900
  };
}

function hitCosmicBlaster(owner) {
  const line = cosmicBlasterLine(owner);
  [opponentOf(owner), ...enemySummonsOf(owner)].forEach(target => {
    if (!target?.hp) return;
    const lineDx = line.x2 - line.x1;
    const lineDy = line.y2 - line.y1;
    const lengthSquared = lineDx * lineDx + lineDy * lineDy || 1;
    const t = clamp(((target.x - line.x1) * lineDx + (target.y - line.y1) * lineDy) / lengthSquared, 0, 1);
...
function drawCosmicBlasters() {
  game.fighters.filter(fighter => fighter.cosmicBlasterActive || fighter.cosmicBlasterCharging > 0).forEach(fighter => {
    ctx.save();
    ctx.globalCompositeOperation = "lighter";
    if (fighter.cosmicBlasterCharging > 0) {
  ...
  game.areaAttacks = game.areaAttacks.filter(attack => {
    ...
    if (attack.delay <= 0 && !attack.hit) {
      ...
      if (attack.type === "supernova") {
        [target, ...enemySummonsOf(attack.owner)].forEach(hitTarget => {
          if (hitTarget?.hp > 0 && Math.hypot(hitTarget.x - attack.x, hitTarget.y - attack.y) < hitTarget.radius + attack.radius) {
            ...
            if (!hitTarget.owner && attack.stun) hitTarget.stunTime = Math.max(hitTarget.stunTime || 0, attack.stun);
          }
        });
      ...
      if (hitsImpactArea || hitsLaserColumn) {
        ...
        if (attack.stun) target.stunTime = Math.max(target.stunTime || 0, attack.stun);
      }
    }
    return attack.life > 0;
  });
...
function drawArtOrbs() {
  ...
  game.artOrbs.forEach(orb => {
    ...
    const stride = Math.max(1, Math.ceil(orb.trail.length / 260));
    for (let index = stride; index < orb.trail.length; index += stride) {
      const previous = orb.trail[index - stride];
      const current = orb.trail[index];
...
function drawDemonMark(target) {
  ...
  ctx.translate(target.x, target.y);
  for (let index = 0; index < count; index += 1) {
    const angle = index / Math.max(1, count) * Math.PI * 2 + game.tick * 0.055;
    const orbit = target.radius + 22 + (index % 2) * 5;
...
function drawAreaAttack(attack) {
  ...
  if (attack.type === "supernova") {
    const progress = attack.delay > 0
      ? 1 - attack.delay / 34
      : 1 - clamp(attack.life / 44, 0, 1);
    ctx.globalCompositeOperation = "lighter";
    ctx.globalAlpha = attack.delay > 0 ? 0.32 + progress * 0.24 : clamp(attack.life / 42, 0, 1);
    ctx.translate(attack.x, attack.y);
    ctx.shadowColor = "#93c5fd";
    ctx.shadowBlur = attack.delay > 0 ? 22 : 48;
    for (let ring = 0; ring < 4; ring += 1) {
      ctx.strokeStyle = ring % 2 ? "#c4b5fd" : "#f8fbff";
      ctx.lineWidth = ring === 0 ? 7 : 3;
      ctx.setLineDash(ring % 2 ? [18, 12] : []);
      ctx.lineDashOffset = -game.tick * (ring + 1);
      ctx.beginPath();
      ctx.arc(0, 0, attack.radius * (0.35 + progress * 0.65) - ring * 10, 0, Math.PI * 2);
      ctx.stroke();
    }
    if (attack.delay <= 0) {
      const core = ctx.createRadialGradient(0, 0, 0, 0, 0, attack.radius * 0.72);
      core.addColorStop(0, "rgba(248,251,255,0.72)");
      core.addColorStop(0.38, "rgba(147,197,253,0.32)");
      core.addColorStop(1, "rgba(67,56,202,0)");
      ctx.fillStyle = core;
      ctx.beginPath();
      ctx.arc(0, 0, attack.radius * 0.72, 0, Math.PI * 2);
      ctx.fill();
    }
    ctx.restore();
    return;
...
function fireSurvivalWeapon(id) {
  ...
  if (id === "pulse") {
    ...
    const count = entry.awakened ? 7 : itemOwned ? 3 : 2;
    for (let index = 0; index < count; index += 1) {
      projectile((index - (count - 1) / 2) * 0.075, entry.awakened ? 28 : itemOwned ? 25 : 22, entry.awakened ? 8.5 : 7.5, 5.5, {
        color: entry.awakened ? "#fde68a" : "#f59e0b",
    ...
    const count = entry.awakened ? 28 : itemOwned ? 18 : 12;
    for (let index = 0; index < count; index += 1) {
      const jitter = (pveRandomIndex(101) - 50) / 1000;
      projectile((index - (count - 1) / 2) * 0.014 + jitter, entry.awakened ? 23 : 19, entry.awakened ? 2 : 1.2, entry.awakened ? 4.8 : 3.8, {
        color: index % 3 === 0 ? "#fde68a" : "#fb923c",
        homing: entry.awakened && index % 4 === 0,
    ...
    const count = entry.awakened ? 5 : 2;
    for (let index = 0; index < count; index += 1) {
      projectile((index - (count - 1) / 2) * 0.12, entry.awakened ? 18 : 16, entry.awakened ? 10 : 7, entry.awakened ? 10 : 7, {
        color: index % 2 ? "#e0f2fe" : "#7dd3fc",
        pierce: entry.awakened ? 4 : itemOwned ? 1 : 0,
        life: 150,
        slow: true,
        visual: "iceShard",
        trail: "#bae6fd",
  ...
  } else if (id === "blizzardEssence") {
    const count = entry.awakened ? 3 : 1;
    for (let index = 0; index < count; index += 1) {
      projectile((index - (count - 1) / 2) * 0.18, entry.awakened ? 9.5 : 7.5, entry.awakened ? 16 : 12, entry.awakened ? 17 : 13, {
        color: "#bae6fd",
        homing: true,
    ...
    const count = entry.awakened ? 3 : 1;
    for (let index = 0; index < count; index += 1) {
      projectile((index - (count - 1) / 2) * 0.16, 21, entry.awakened ? 13 : 9, itemOwned ? 15 : 11, {
        color: "#d6a35d",
        pierce: entry.awakened ? 8 : 2,
        life: 105,
        visual: "rope",
    ...
    const jackpot = entry.awakened || (itemOwned && pveRandomIndex(100) < 18);
    const roll = pveRandomIndex(4);
    if (roll === 0) {
      const count = jackpot ? 5 : 2;
      for (let index = 0; index < count; index += 1) {
        projectile((index - (count - 1) / 2) * 0.13, 18, jackpot ? 12 : 8, 8, {
          color: index % 2 ? "#facc15" : "#fb7185",
          pierce: jackpot ? 3 : 0,
          life: 140,
          visual: "card",
          label: "?",
      ...
      pveGame.areaAttacks.push({
        x: player.x, y: player.y, x1: player.x, y1: player.y,
        x2: player.x + Math.cos(angle) * length,
        y2: player.y + Math.sin(angle) * length,
        radius: (jackpot ? 34 : 21) * stats.sizeScale,
        damage: (jackpot ? 22 : 12) * stats.damageScale,
        delay: 8, life: 32, hit: false, color: "#a78bfa", type: "lineLaser", survival: true
      });
    }
  } else if (id === "ultimateRoulette") {
    const repeats = entry.awakened ? 6 : itemOwned ? 4 : 3;
    ...
    const count = entry.awakened ? 3 : 1;
    for (let index = 0; index < count; index += 1) {
      const novaX = entry.awakened ? clamp(target.x + (index - 1) * 110, 80, pveCanvas.width - 80) : target.x;
      pveGame.areaAttacks.push({
        x: novaX,
        y: target.y,
        radius: (entry.awakened ? 170 : itemOwned ? 135 : 110) * stats.sizeScale,
        damage: (entry.awakened ? 25 : 15) * stats.damageScale,
        delay: 34 + index * 12,
        life: 72 + index * 12,
        hit: false,
  ...
  } else if (id === "cosmicBlaster") {
    const count = entry.awakened ? 3 : 1;
    const length = Math.hypot(pveCanvas.width, pveCanvas.height) * 1.25;
    for (let index = 0; index < count; index += 1) {
      const beamAngle = angle + (index - (count - 1) / 2) * 0.14;
      pveGame.areaAttacks.push({
        x: player.x, y: player.y, x1: player.x, y1: player.y,
        x2: player.x + Math.cos(beamAngle) * length,
        y2: player.y + Math.sin(beamAngle) * length,
        radius: (entry.awakened ? 48 : itemOwned ? 38 : 30) * stats.sizeScale,
        damage: (entry.awakened ? 24 : 14) * stats.damageScale,
        delay: 18 + index * 5,
        life: 56 + index * 5,
        hit: false,
  ...
  } else if (id === "hackingBullet") {
    const count = entry.awakened ? 3 : itemOwned ? 2 : 1;
    for (let index = 0; index < count; index += 1) {
      projectile((index - (count - 1) / 2) * 0.1, entry.awakened ? 25 : 22, entry.awakened ? 12 : 8, 7, {
        color: "#34d399",
    ...
    const length = Math.hypot(target.x - player.x, target.y - player.y);
    pveGame.areaAttacks.push({
      x: player.x, y: player.y, x1: player.x, y1: player.y,
      x2: player.x + Math.cos(angle) * Math.max(220, length + 80),
      y2: player.y + Math.sin(angle) * Math.max(220, length + 80),
      radius: (entry.awakened ? 18 : 12) * stats.sizeScale,
      damage: (entry.awakened ? 10 : 6) * stats.damageScale,
      delay: 16,
      life: 38,
      hit: false,
      color: "#34d399",
  ...
  } else if (id === "hologramLaser") {
    const count = entry.awakened ? 5 : itemOwned ? 3 : 2;
    const length = Math.hypot(pveCanvas.width, pveCanvas.height) * 1.1;
    for (let index = 0; index < count; index += 1) {
      const offsetAngle = angle + Math.PI / 2;
      const offset = (index - (count - 1) / 2) * 72;
      const startX = clamp(player.x + Math.cos(offsetAngle) * offset, 25, pveCanvas.width - 25);
      const startY = clamp(player.y + Math.sin(offsetAngle) * offset, 25, pveCanvas.height - 25);
      const beamAngle = Math.atan2(target.y - startY, target.x - startX);
      pveGame.areaAttacks.push({
        x: startX, y: startY, x1: startX, y1: startY,
        x2: startX + Math.cos(beamAngle) * length,
        y2: startY + Math.sin(beamAngle) * length,
        radius: (entry.awakened ? 20 : 14) * stats.sizeScale,
  ...
  } else if (id === "rockShard") {
    const count = entry.awakened ? 5 : itemOwned ? 3 : 2;
    for (let index = 0; index < count; index += 1) {
      projectile((index - (count - 1) / 2) * 0.13, entry.awakened ? 14 : 12, entry.awakened ? 18 : 12, entry.awakened ? 13 : 10, {
        color: index % 2 ? "#78716c" : "#a16207",
        pierce: entry.awakened ? 5 : itemOwned ? 2 : 0,
        life: 180,
        visual: "rockShard",
        trail: "#a16207",
    ...
    for (let index = 0; index < count; index += 1) {
      const offset = (index - (count - 1) / 2) * 88;
      const centerX = clamp(target.x + Math.cos(wallAngle) * offset, 70, pveCanvas.width - 70);
      const centerY = clamp(target.y + Math.sin(wallAngle) * offset, 70, pveCanvas.height - 70);
      pveGame.areaAttacks.push({
        x: centerX,
        y: centerY,
        x1: centerX,
        y1: centerY - (entry.awakened ? 145 : 105) * stats.sizeScale,
        x2: centerX,
        y2: centerY + (entry.awakened ? 145 : 105) * stats.sizeScale,
        radius: (entry.awakened ? 38 : 28) * stats.sizeScale,
        damage: (entry.awakened ? 22 : 12) * stats.damageScale,
        delay: 28 + index * 8,
        life: 72 + index * 8,
        hit: false,
...
function spawnSurvivalBoss() {
  ...
  pveGame.enemies.push({
    ...
    vx: 0,
    vy: 1.25,
    baseSpeed: (0.92 + count * 0.06) * mode.enemySpeed,
  ...
  addPveFloating(count >= 3 ? "종말 보스 출현!" : "균열 파수꾼 출현! 시간이 멈춥니다", "#fda4af");
}
...
function damageSurvivalEnemy(enemy, amount, sourceWeaponId = "") {
  ...
  const lifestealScale = enemy.boss ? 0.1 : 1;
  if (pveGame.items.chalice) healPvePlayer(actual * 0.12 * lifestealScale);
  if (sourceWeaponId === "deathSword" && pveGame.items.demonSigil) healPvePlayer(actual * 0.1 * lifestealScale);
  if (enemy.hp <= 0 && !enemy.dead) {
    enemy.dead = true;
    pveGame.kills += 1;
    dropSurvivalXp(enemy);
    if (enemy.boss && pveGame.reaperAfterBoss) {
      startSurvivalReaper();
      pveGame.reaperAfterBoss = false;
    }
    if (enemy.miniBoss) {
...
function activeSurvivalBoss() {
  return pveGame?.enemies?.some(enemy => enemy.boss && !enemy.dead) || false;
}

function startSurvivalReaper() {
  if (!pveGame || pveGame.reaperActive) return;
  const player = pveGame.player;
  ...
  pveGame.projectiles = [];
  pveGame.areaAttacks = [];
  addPveFloating("사신이 도착했습니다", "#ef4444");
}
...
function stepSurvivalPve() {
  ...
  const seconds = pveGame.tick / 60;
  if (pveGame.tick >= 54000 && pveGame.bossCount >= 3 && !pveGame.reaperAfterBoss) {
    startSurvivalReaper();
    updateSurvivalHud();
    return;
  }
  ...
  if (pveGame.spawnTimer <= 0) {
    const mode = pveGame.difficulty || SURVIVAL_DIFFICULTIES.easy;
    const lateRush = pveGame.bossCount >= 1 || seconds >= 420;
    const endgame = pveGame.bossCount >= 2 || seconds >= 600;
    const baseCount = endgame
      ? 2 + Math.floor((seconds - 600) / 150)
      : lateRush
      ? 5 + Math.floor((seconds - 420) / 90)
      : 1 + Math.floor(Math.max(0, seconds - 210) / 120);
    const count = bossTimePaused ? 1 : Math.ceil(baseCount * mode.spawnCount);
    ...
    const enemyCap = bossTimePaused ? 12 : Math.min(endgame ? 78 : 140, Math.floor(baseEnemyCap * mode.enemyCap));
    const availableSlots = Math.max(0, enemyCap - pveGame.enemies.filter(enemy => !enemy.dead).length);
    const waveLimit = bossTimePaused ? 1 : Math.ceil((endgame ? 3 : lateRush ? 9 : 4) * mode.spawnCount);
    ...
    pveGame.spawnTimer = bossTimePaused ? 210 : Math.max(10, baseInterval * mode.spawnInterval);
  }
    ...
    if (enemy.attackTimer < Infinity) {
      ...
      if (enemy.attackTimer <= 0) {
        if (enemy.type === "survivalBoss") {
          const mode = pveGame.difficulty || SURVIVAL_DIFFICULTIES.easy;
          const phase = (enemy.bossIndex || pveGame.bossCount) % 3;
          if (phase === 1) {
            const bulletCount = pveGame.difficultyId === "hard" ? 18 : pveGame.difficultyId === "normal" ? 15 : 12;
            for (let index = 0; index < bulletCount; index += 1) {
              ...
              spawnSurvivalProjectile({
                ...
                damage: (10 + pveGame.bossCount * 1.5) * mode.enemyDamage,
                radius: 9,
                color: "#fb7185",
                life: 240
              });
            ...
            [[0, 0], [110, -70], [-110, 70], [0, 135]].forEach(([ox, oy], index) => {
              pveGame.enemyAreas.push({
                x: clamp(player.x + ox, 55, pveCanvas.width - 55),
                y: clamp(player.y + oy, 55, pveCanvas.height - 55),
                radius: index === 0 ? 88 : 68,
                damage: (18 + pveGame.bossCount * 2) * mode.enemyDamage,
                delay: 58 + index * 10,
                life: 90 + index * 10,
                color: "#fb7185",
                stun: 18
            ...
            for (let index = 0; index < 5; index += 1) {
              const offset = (index - 2) * 84;
              pveGame.areaAttacks.push({
                x1: clamp(player.x + offset, 30, pveCanvas.width - 30),
                y1: 0,
                x2: clamp(player.x + offset, 30, pveCanvas.width - 30),
                y2: pveCanvas.height,
                radius: 26,
                damage: (16 + pveGame.bossCount * 2) * mode.enemyDamage,
                delay: 42 + index * 8,
                life: 78 + index * 8,
                hit: false,
                color: "#fda4af",
  ...
  pveGame.projectiles = pveGame.projectiles.filter(projectile => {
    ...
    if (projectile.owner === "enemy") {
      ...
      if (enemy) {
        ...
        if (projectile.stun) enemy.stunTime = Math.max(enemy.stunTime, projectile.stun);
  ...
  pveGame.enemyAreas = pveGame.enemyAreas.filter(attack => {
    ...
    if (attack.delay <= 0 && !attack.hit) {
      attack.hit = true;
      if (Math.hypot(player.x - attack.x, player.y - attack.y) < player.radius + attack.radius) {
        if (player.invulnerableTime <= 0) damagePvePlayer(attack.damage);
        if (attack.stun) player.stunTime = Math.max(player.stunTime || 0, attack.stun);
        player.invulnerableTime = Math.max(player.invulnerableTime, 18);
      }
    }
    return attack.life > 0;
  });
...
function drawSurvivalAreaAttack(attack) {
  ...
  if (visual.startsWith("explosionBoost") || visual.startsWith("houserImpact")) {
    pveCtx.translate(attack.x, attack.y);
    const blast = pveCtx.createRadialGradient(0, 0, 0, 0, 0, attack.radius);
    blast.addColorStop(0, "#fff7ed");
    blast.addColorStop(0.22, "#facc15");
    blast.addColorStop(0.48, attack.color);
    blast.addColorStop(0.72, "#7f1d1d");
    blast.addColorStop(1, "rgba(0,0,0,0)");
    pveCtx.fillStyle = blast;
    pveCtx.beginPath();
    pveCtx.arc(0, 0, attack.radius * (0.38 + progress * 0.58), 0, Math.PI * 2);
    pveCtx.fill();
    pveCtx.strokeStyle = "#fff7ed";
    pveCtx.lineWidth = attack.awakened ? 5 : 3;
    for (let shard = 0; shard < (attack.awakened ? 18 : 11); shard += 1) {
      const angle = shard * Math.PI * 2 / (attack.awakened ? 18 : 11) + pveGame.tick * 0.014;
      const inner = attack.radius * (0.16 + progress * 0.12);
      const outer = attack.radius * (0.78 + Math.sin(pveGame.tick * 0.11 + shard) * 0.1);
      pveCtx.beginPath();
      pveCtx.moveTo(Math.cos(angle) * inner, Math.sin(angle) * inner);
      pveCtx.lineTo(Math.cos(angle) * outer, Math.sin(angle) * outer);
      pveCtx.stroke();
    }
    pveCtx.restore();
    return;
  ...
  if (visual.startsWith("supernova") || visual.startsWith("ultimateRoulette")) {
    pveCtx.translate(attack.x, attack.y);
    pveCtx.rotate(pveGame.tick * 0.012);
    const rings = attack.awakened ? 7 : 5;
    for (let ring = 0; ring < rings; ring += 1) {
      const rate = ring / rings;
      pveCtx.globalAlpha = alpha * (0.82 - rate * 0.45);
      pveCtx.strokeStyle = ring % 2 ? "#f7fbff" : attack.color;
      pveCtx.lineWidth = attack.awakened ? 5 : 3;
      pveCtx.setLineDash(ring % 2 ? [18, 12] : [7, 9]);
      pveCtx.lineDashOffset = -pveGame.tick * (1 + ring * 0.25);
      pveCtx.beginPath();
      pveCtx.arc(0, 0, attack.radius * (0.25 + rate * 0.64 + progress * 0.16), 0, Math.PI * 2);
      pveCtx.stroke();
    }
    pveCtx.setLineDash([]);
    const core = pveCtx.createRadialGradient(0, 0, 0, 0, 0, attack.radius * 0.56);
    core.addColorStop(0, "rgba(255,255,255,0.95)");
    core.addColorStop(0.32, `${attack.color}dd`);
    core.addColorStop(1, "rgba(0,0,0,0)");
    pveCtx.fillStyle = core;
    pveCtx.beginPath();
    pveCtx.arc(0, 0, attack.radius * (0.32 + progress * 0.16), 0, Math.PI * 2);
    pveCtx.fill();
    pveCtx.restore();
    return;
  }
  ...
  pveCtx.translate(attack.x, attack.y);
  pveCtx.strokeStyle = attack.color;
  pveCtx.fillStyle = `${attack.color}${attack.awakened ? "34" : "1f"}`;
  pveCtx.lineWidth = warning ? 3 : attack.awakened ? 10 : 7;
  pveCtx.setLineDash(warning ? [8, 7] : []);
  for (let ring = 0; ring < (warning ? 2 : attack.awakened ? 5 : 4); ring += 1) {
    const ringRate = ring / (warning ? 2 : attack.awakened ? 5 : 4);
    pveCtx.globalAlpha = alpha * (0.9 - ringRate * 0.45);
    pveCtx.beginPath();
    pveCtx.arc(0, 0, attack.radius * (0.48 + ringRate * 0.42 + progress * 0.16), 0, Math.PI * 2);
    if (ring === 0) pveCtx.fill();
    pveCtx.stroke();
  }
  if (!warning) {
    pveCtx.globalAlpha = alpha * 0.72;
    pveCtx.strokeStyle = "#f7fbff";
    pveCtx.lineWidth = attack.awakened ? 3 : 2;
    pveCtx.setLineDash([16, 12]);
    pveCtx.lineDashOffset = -pveGame.tick * 2;
    for (let spoke = 0; spoke < (attack.awakened ? 18 : 12); spoke += 1) {
      const angle = spoke * Math.PI * 2 / (attack.awakened ? 18 : 12) + pveGame.tick * 0.018;
      pveCtx.beginPath();
      pveCtx.moveTo(Math.cos(angle) * attack.radius * 0.22, Math.sin(angle) * attack.radius * 0.22);
      pveCtx.lineTo(Math.cos(angle) * attack.radius * (1.02 + progress * 0.16), Math.sin(angle) * attack.radius * (1.02 + progress * 0.16));
      pveCtx.stroke();
    }
    const core = pveCtx.createRadialGradient(0, 0, 0, 0, 0, attack.radius * 0.55);
    core.addColorStop(0, "rgba(255,255,255,0.9)");
    core.addColorStop(0.35, `${attack.color}cc`);
    core.addColorStop(1, "rgba(255,255,255,0)");
    pveCtx.fillStyle = core;
    pveCtx.beginPath();
    pveCtx.arc(0, 0, attack.radius * 0.55, 0, Math.PI * 2);
    pveCtx.fill();
  }
  pveCtx.restore();
}
...
function drawSurvivalPve() {
  ...
  pveGame.enemyAreas.forEach(attack => {
    ...
    pveCtx.save();
    pveCtx.globalCompositeOperation = "lighter";
    pveCtx.globalAlpha = warning ? 0.32 + Math.sin(pveGame.tick * 0.28) * 0.1 : Math.min(1, attack.life / 24);
    pveCtx.strokeStyle = attack.color;
    pveCtx.fillStyle = `${attack.color}${warning ? "18" : "30"}`;
    pveCtx.shadowColor = attack.color;
    pveCtx.shadowBlur = warning ? 16 : 36;
    pveCtx.lineWidth = warning ? 3 : 10;
    pveCtx.setLineDash(warning ? [12, 9] : []);
    pveCtx.beginPath();
    pveCtx.arc(attack.x, attack.y, attack.radius, 0, Math.PI * 2);
    pveCtx.fill();
    pveCtx.stroke();
    if (!warning) {
      for (let ray = 0; ray < 12; ray += 1) {
        const angle = ray * Math.PI * 2 / 12 + pveGame.tick * 0.02;
        pveCtx.beginPath();
        pveCtx.moveTo(attack.x + Math.cos(angle) * attack.radius * 0.3, attack.y + Math.sin(angle) * attack.radius * 0.3);
        pveCtx.lineTo(attack.x + Math.cos(angle) * attack.radius * 1.08, attack.y + Math.sin(angle) * attack.radius * 1.08);
        pveCtx.stroke();
      }
    }
    pveCtx.restore();
  ...
  pveGame.projectiles.forEach(projectile => {
    ...
    if (projectile.visual === "card") {
    ...
    } else if (projectile.visual === "bullet") {
      const length = projectile.radius * 4.4 + speed * 0.3;
      const gradient = pveCtx.createLinearGradient(-length * 0.5, 0, length * 0.5, 0);
      gradient.addColorStop(0, "rgba(255,255,255,0)");
      gradient.addColorStop(0.25, projectile.color);
      gradient.addColorStop(0.72, "#fff7d6");
      gradient.addColorStop(1, "#ffffff");
      pveCtx.fillStyle = gradient;
      pveCtx.beginPath();
      pveCtx.roundRect(-length * 0.45, -projectile.radius * 0.45, length, projectile.radius * 0.9, projectile.radius * 0.45);
      pveCtx.fill();
      pveCtx.fillStyle = "#fff7ed";
      pveCtx.beginPath();
      pveCtx.arc(length * 0.48, 0, projectile.radius * 0.58, 0, Math.PI * 2);
      pveCtx.fill();
    } else if (projectile.visual === "iceShard") {
      const length = projectile.radius * 3.4;
      pveCtx.fillStyle = "#e0f2fe";
      pveCtx.strokeStyle = projectile.color;
      pveCtx.lineWidth = 2;
      pveCtx.beginPath();
      pveCtx.moveTo(length * 0.58, 0);
      pveCtx.lineTo(-length * 0.22, -projectile.radius * 0.88);
      pveCtx.lineTo(-length * 0.58, 0);
      pveCtx.lineTo(-length * 0.22, projectile.radius * 0.88);
      pveCtx.closePath();
      pveCtx.fill();
      pveCtx.stroke();
      pveCtx.globalAlpha *= 0.55;
      pveCtx.strokeStyle = "#ffffff";
      pveCtx.beginPath();
      pveCtx.moveTo(-length * 0.36, 0);
      pveCtx.lineTo(length * 0.42, 0);
      pveCtx.stroke();
    } else if (projectile.visual === "hacking") {
      const size = projectile.radius * 2.1;
      pveCtx.strokeStyle = "#d1fae5";
      pveCtx.lineWidth = 2.2;
      pveCtx.rotate(-angle + pveGame.tick * 0.12);
      pveCtx.strokeRect(-size * 0.5, -size * 0.5, size, size);
      pveCtx.rotate(Math.PI / 4);
      pveCtx.strokeRect(-size * 0.42, -size * 0.42, size * 0.84, size * 0.84);
      pveCtx.fillStyle = projectile.color;
      for (let node = 0; node < 4; node += 1) {
        const nodeAngle = node * Math.PI / 2 + pveGame.tick * 0.08;
        pveCtx.beginPath();
        pveCtx.arc(Math.cos(nodeAngle) * size * 0.48, Math.sin(nodeAngle) * size * 0.48, 2.4, 0, Math.PI * 2);
        pveCtx.fill();
      }
    } else if (projectile.visual === "rope") {
      const length = projectile.radius * 5;
      pveCtx.strokeStyle = "#fbbf24";
      pveCtx.lineWidth = 3;
      pveCtx.setLineDash([6, 5]);
      pveCtx.lineDashOffset = -pveGame.tick * 2;
      pveCtx.beginPath();
      pveCtx.moveTo(-length * 0.65, 0);
      for (let step = 0; step <= 6; step += 1) {
        const t = step / 6;
        pveCtx.lineTo(-length * 0.65 + length * t, Math.sin(t * Math.PI * 3 + pveGame.tick * 0.18) * projectile.radius * 0.2);
      }
      pveCtx.stroke();
      pveCtx.setLineDash([]);
      pveCtx.fillStyle = "#d6a35d";
      pveCtx.beginPath();
      pveCtx.moveTo(length * 0.54, 0);
      pveCtx.lineTo(length * 0.22, -projectile.radius * 0.82);
      ...
      pveCtx.strokeStyle = "#e7c78e";
      pveCtx.lineWidth = 2;
      pveCtx.beginPath();
      pveCtx.moveTo(length * 0.62, 0);
      pveCtx.lineTo(projectile.radius * 0.2, -projectile.radius);
      pveCtx.lineTo(-length * 0.48, -projectile.radius * 0.36);
      pveCtx.lineTo(-length * 0.36, projectile.radius * 0.72);
      pveCtx.lineTo(projectile.radius * 0.28, projectile.radius * 0.94);
      pveCtx.closePath();
      pveCtx.fill();
      pveCtx.stroke();
      pveCtx.strokeStyle = "#292524";
      pveCtx.globalAlpha *= 0.35;
      pveCtx.beginPath();
      pveCtx.moveTo(-length * 0.25, -projectile.radius * 0.25);
      pveCtx.lineTo(projectile.radius * 0.4, projectile.radius * 0.26);
      pveCtx.stroke();
...
ui.mailboxButton?.addEventListener("click", () => setMailboxOpen(true));
ui.mailboxCloseButton?.addEventListener("click", () => setMailboxOpen(false));
